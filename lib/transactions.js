...
exports.createGeneration = (rpcData, blockReward, feeReward, recipients, poolAddress, poolHex, coin, masternodeReward, masternodePayee, masternodePayment) => {
    ...
    console.log(rpcData.height)
    console.log('sapling ' + coin.sapling)
    console.log('overwinter ' + coin.overwinter)

   //if (coin.overwinter)
   //{
  //    console.log('overwinter level1')
    if (coin.sapling === true || (typeof coin.sapling === 'number' && coin.sapling <= rpcData.height))
    {
      ...
      console.log('setting sapling version ' + bitcoin.Transaction.ZCASH_SAPLING_VERSION )
    }
    elseif (coin.overwinter === true || (typeof coin.overwinter === 'number' && coin.overwinter <= rpcData.height))
    {
        txb.setVersion(bitcoin.Transaction.ZCASH_OVERWINTER_VERSION);
        console.log('setting overwinter version ' + bitcoin.Transaction.ZCASH_OVERWINTER_VERSION)
    }
    //}

    //if (coin.sapling)
    //{
    //  console.log('sapling level1')
