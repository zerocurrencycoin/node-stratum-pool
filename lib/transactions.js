...
exports.createGeneration = (rpcData, blockReward, feeReward, recipients, poolAddress, poolHex, coin, masternodeReward, masternodePayee, masternodePayment) => {
    ...
    if (coin.saplingActivationHight  === true || (typeof coin.saplingActivationHight  === 'number' && coin.saplingActivationHight  <= rpcData.height))
    {
      txb.setVersion(bitcoin.Transaction.ZCASH_SAPLING_VERSION);
    }
    else if (coin.overwinterActivationHight === true || (typeof coin.overwinterActivationHight === 'number' && coin.overwinterActivationHight<= rpcData.height))
    {
